document.addEventListener('DOMContentLoaded', function() {
    ...
    function displayResults(cities) {
        ...
        if (!Array.isArray(cities)) {
            resultHTML += '<p>Unexpected response from the server.</p>';
        } else if (cities.length === 0) {
            resultHTML += '<p>No cities match your criteria. Please try adjusting your preferences.</p>';
        } else {
            resultHTML += '<div class="row">';
            cities.forEach((city, index) => {
                const matchingTypes = Array.isArray(city.matching_types) ? city.matching_types : [];
                resultHTML += `
                    <div class="col-md-4 mb-4">
                        <div class="card h-100">
                            <div class="card-body">
                                ...
                                <ul>
                                    ${matchingTypes.map(type => `<li>${type}</li>`).join('')}
                                </ul>
                            </div>
                        </div>
                    </div>
