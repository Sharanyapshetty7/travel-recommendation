...
# Load datasets
try:
    states_df = pd.read_csv('datasets/states_and_union_territories.csv')
    cities_df = pd.read_csv('datasets/cities.csv')
    budget_duration_df = pd.read_csv('datasets/city_budget_duration.csv')
    cities_type_df = pd.read_csv('datasets/cities_type_data.csv')
...
def get_cities():
    ...
    try:
        ...
        final_cities['match_score'] = final_cities['Type_ID'].apply(
            lambda x: len(set(x) & set(experience_types)) / len(experience_types) * 100
        )
        ...
        # ✅ Fix applied here
        result = list(final_cities.apply(lambda row: {
            'name': row['City_Name'],
